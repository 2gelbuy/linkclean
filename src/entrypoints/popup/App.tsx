...
import { getFilters, updateFilters, type FilterSettings, DEFAULT_FILTERS } from '@/lib/filters';
...
export default function App() {
  ...
  const handleToggle = useCallback(async (key: keyof FilterSettings, value: boolean) => {
    ...
    <div className="w-[340px] bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100">
      ...
      <div className="px-4 py-3 bg-gray-50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-xs font-medium text-gray-500 uppercase tracking-wider">Posts Hidden</span>
        </div>
        <div className="flex gap-6">
          <div>
            <span className="text-2xl font-bold text-blue-600">{filters.sessionHiddenCount}</span>
            <span className="text-xs text-gray-500 ml-1">this session</span>
          </div>
          <div>
            <span className="text-2xl font-bold text-gray-700 dark:text-gray-300">{filters.hiddenCount}</span>
      ...
      {sessionCount >= 5 && (
        <footer className="px-4 py-2 border-t border-gray-200 dark:border-gray-700 text-center">
          <a
            href="https://chromewebstore.google.com/detail/linkclean/CHANGE_CWS_ID/reviews"
            target="_blank"
